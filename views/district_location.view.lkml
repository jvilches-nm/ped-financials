view: district_location {
  derived_table: {
   sql: select * from looker.district_location where district_organization_type in ('State District', 'State Charter', 'State Supported') and location_status in ('OPEN', 'NEW') and location_organization_type not in ('BIA', 'Home School', 'Private');;
  }
