- dashboard: school_comp
  ...
  - name: Avg Spending per Student to Date
    ...
    note_display: hover
    note_text: |-
      Total school expenditures to date and total school expenditures to date for the Instructional function for the selected school year not including capital or debt service funds divided by the number of school students based on the 40-day count for that school year. This average should be considered an estimate for comparison purposes and not an actual tally of the spending per student. Off-site students are not included in these calculations.
    merged_queries:
    - model: ped_public_financials
      explore: actuals_line
      type: table
